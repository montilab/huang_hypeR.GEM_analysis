---
title: "COVID Urine Summary Tables"
author: "Ziwei Huang"
...
protein_diffanal <- readRDS(file = file.path(PATH,"/data/External/COVID_urine_protein.rds")) %>% 
  ...
  dplyr::filter(P_value_adjust <= max_fdr)

met_diffanal <- readRDS(file = file.path(PATH,"/data/External/COVID_urine_mets.rds")) %>% 
  do.call(rbind,.) %>% 
...
### non-directional
hypeR_GEM_obj <- readRDS(file = file.path(PATH, "data/External/urine_hypR_object.rds"))
...
### directional
hypeR_GEM_obj_di <- readRDS(file = file.path(PATH, "data/External/urine_hypR_object_di.rds"))
...
write.xlsx(res, file = file.path(PATH, "results/tables/COVID_urine_summary_tables.xlsx"))
```
...
```{r}
hallmark_files <- list.files(file.path(PATH, "data/External/enrichment/urine"), pattern = "hallmark\\.rds$", full.names = TRUE)
kegg_files <- list.files(file.path(PATH, "data/External/enrichment/urine"), pattern = "kegg\\.rds$", full.names = TRUE)
reactome_files <- list.files(file.path(PATH, "data/External/enrichment/urine"), pattern = "reactome\\.rds$", full.names = TRUE)
gobp_files <- list.files(file.path(PATH, "data/External/enrichment/urine"), pattern = "gobp\\.rds$", full.names = TRUE)
gomf_files <- list.files(file.path(PATH, "data/External/enrichment/urine"), pattern = "gomf\\.rds$", full.names = TRUE)
```
...
write.xlsx(GEM_enrichment_res, file = file.path(PATH, "results/tables/COVID_urine_GEM_enrichment.xlsx"))
```
