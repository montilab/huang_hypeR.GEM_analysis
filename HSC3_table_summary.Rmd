...
met_diffanal <- readRDS(file.path(PATH,"/data/HSC3/metabolite_diffanal.rds")) %>% 
  ...
  dplyr::filter(`adj.P.Val` <= max_fdr)
...
### non-directional
hypeR_GEM_obj <- readRDS(file = file.path(PATH, "data/HSC3/hypR_object.rds"))
...
### directional
hypeR_GEM_obj_di <- readRDS(file = file.path(PATH, "data/HSC3/hypR_object_di.rds"))
...
res <- list("Diffanal mRNA" = mRNA_diffanal,
            "Diffanal Metabolomics" = met_diffanal,
            "GEM-mapped Non-directional" = gene_table,
            "GEM-mapped Directional" = gene_table_di)

write.xlsx(res, file = file.path(PATH, "results/tables/HSC3_summary_tables.xlsx"))
```
...
```{r}
hallmark_files <- list.files(file.path(PATH, "data/HSC3/enrichment"), pattern = "hallmark\\.rds$", full.names = TRUE)
kegg_files <- list.files(file.path(PATH, "data/HSC3/enrichment"), pattern = "kegg\\.rds$", full.names = TRUE)
reactome_files <- list.files(file.path(PATH, "data/HSC3/enrichment"), pattern = "reactome\\.rds$", full.names = TRUE)
gobp_files <- list.files(file.path(PATH, "data/HSC3/enrichment"), pattern = "gobp\\.rds$", full.names = TRUE)
gomf_files <- list.files(file.path(PATH, "data/HSC3/enrichment"), pattern = "gomf\\.rds$", full.names = TRUE)
```
...
write.xlsx(GEM_enrichment_res, file = file.path(PATH, "results/tables/HSC3_GEM_enrichment.xlsx"))
```