...
source(file.path(PATH,"R/read_multiple_sheets.R"))
```
...
met_diffanal <-  read_multiple_sheets(file.path(PATH,"data/NECS/supp_tables_formatted.xlsx"),
                            excluded = "Annotation")

## Only use Table S1(age), S2(EL)
met_diffanal <- met_diffanal$Table_S1 %>% 
  dplyr::arrange(`age.p.value`) %>% 
  dplyr::filter(`age.q.value` <= max_fdr)

## protein
protein_diffanal <-  readRDS(file = file.path(PATH,"data/NECS/lmRes.RDS")) %>% 
  dplyr::mutate(in_GEM = case_when(
    geneID %in% GEM_gene_map$symbol ~ TRUE,
    TRUE ~ FALSE
  )) %>%
  dplyr::arrange(`Pr(>|t|)`) %>% 
...
write.xlsx(res, file = file.path(PATH, "results/tables/NECS_Age_summary_tables.xlsx"))
```
...
```{r}
hallmark_files <- list.files(file.path(PATH, "data/NECS/enrichment/age"), pattern = "hallmark\\.rds$", full.names = TRUE)
kegg_files <- list.files(file.path(PATH, "data/NECS/enrichment/age"), pattern = "kegg\\.rds$", full.names = TRUE)
reactome_files <- list.files(file.path(PATH, "data/NECS/enrichment/age"), pattern = "reactome\\.rds$", full.names = TRUE)
gobp_files <- list.files(file.path(PATH, "data/NECS/enrichment/age"), pattern = "gobp\\.rds$", full.names = TRUE)
gomf_files <- list.files(file.path(PATH, "data/NECS/enrichment/age"), pattern = "gomf\\.rds$", full.names = TRUE)
...
write.xlsx(GEM_enrichment_res, file = file.path(PATH, "results/tables/NECS_Age_GEM_enrichment.xlsx"))
```