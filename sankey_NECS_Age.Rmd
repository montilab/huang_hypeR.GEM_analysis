...
## temporary save
saveRDS(metabolon_msets, file.path(PATH, "data/necs_age_metabolon_msets_list.rds"))
...
top <- 10
gene_layer_enrich_top <- gene_layer_enrich %>%
...
gsets_list <- lapply(gsets_list, function(x,enriched_pathways){return(x[names(x)%in%enriched_pathways])}, enriched_pathways = gene_layer_enrich_top$label)

saveRDS(gsets_list, file.path(PATH, "data/necs_age_gsets_list.rds"))
```

## Organize enriched MSets and GSets
```{r}
hallmark_enrich <- gene_layer_enrich %>% 
  dplyr::filter(set_name == "hallmark") %>% 
  dplyr::select(-c("set_name", "full_label"))

kegg_enrich <- gene_layer_enrich %>% 
  dplyr::filter(set_name == "kegg") %>% 
  dplyr::select(-c("set_name", "full_label"))

reactome_enrich <- gene_layer_enrich %>% 
  dplyr::filter(set_name == "reactome") %>% 
  dplyr::select(-c("set_name", "full_label"))

metabolon_enrich <- metabolon_hyp_enrich %>% 
  dplyr::select(-c("set_name"))

refmet_enrich <- refmet_hyp_enrich %>% 
  dplyr::select(-c("set_name"))

## save to excel spreesheet
res <- list("Metabolon" = metabolon_enrich,
            "Refmet" = refmet_enrich,
            "HALLMARK" = hallmark_enrich,
            "KEGG" = kegg_enrich,
            "REACTOME" = reactome_enrich)

write.xlsx(res, file = file.path(PATH, "results/tables/NECS_Age_enrichment.xlsx"))
```
