...
met_diffanal <- met_diffanal$Table_S2 %>% 
  dplyr::arrange(`cent.p.value`) %>% 
  dplyr::filter(`cent.q.value` <= max_fdr)
...
write.xlsx(res, file = file.path(PATH, "results/tables/NECS_EL_summary_tables.xlsx"))
```
...
```{r}
hallmark_files <- list.files(file.path(PATH, "data/NECS/enrichment/EL"), pattern = "hallmark\\.rds$", full.names = TRUE)
kegg_files <- list.files(file.path(PATH, "data/NECS/enrichment/EL"), pattern = "kegg\\.rds$", full.names = TRUE)
reactome_files <- list.files(file.path(PATH, "data/NECS/enrichment/EL"), pattern = "reactome\\.rds$", full.names = TRUE)
gobp_files <- list.files(file.path(PATH, "data/NECS/enrichment/EL"), pattern = "gobp\\.rds$", full.names = TRUE)
gomf_files <- list.files(file.path(PATH, "data/NECS/enrichment/EL"), pattern = "gomf\\.rds$", full.names = TRUE)
```
...
write.xlsx(GEM_enrichment_res, file = file.path(PATH, "results/tables/NECS_EL_GEM_enrichment.xlsx"))
```