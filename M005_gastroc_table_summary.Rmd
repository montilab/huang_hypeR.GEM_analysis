---
title: "M005 Gastroc summary table"
author: "Ziwei Huang"
...
met_diffanal <- readRDS(file = file.path(PATH, "data/M005/metabolomics/gastroc_met_diffanal.rds")) %>% 
  dplyr::arrange(pvalue) %>% 
  dplyr::filter(fdr <= max_fdr)
...
### non-directional
hypeR_GEM_obj <- readRDS(file = file.path(PATH, "data/M005/gastroc_hypR_GEM_object.rds"))
...
### Directional
hypeR_GEM_obj_di <- readRDS(file = file.path(PATH, "data/M005/gastroc_hypR_GEM_object_di.rds"))
...
write.xlsx(res, file = file.path(PATH, "results/tables/M005_gastroc_summary_tables.xlsx"))
```
...
```{r}
hallmark_files <- list.files(file.path(PATH, "data/M005/gastroc_enrichment"), pattern = "hallmark\\.rds$", full.names = TRUE)
kegg_files <- list.files(file.path(PATH, "data/M005/gastroc_enrichment"), pattern = "kegg\\.rds$", full.names = TRUE)
reactome_files <- list.files(file.path(PATH, "data/M005/gastroc_enrichment"), pattern = "reactome\\.rds$", full.names = TRUE)
gobp_files <- list.files(file.path(PATH, "data/M005/gastroc_enrichment"), pattern = "gobp\\.rds$", full.names = TRUE)
gomf_files <- list.files(file.path(PATH, "data/M005/gastroc_enrichment"), pattern = "gomf\\.rds$", full.names = TRUE)
```
...
write.xlsx(GEM_enrichment_res, file = file.path(PATH, "results/tables/M005_gastroc_GEM_enrichment.xlsx"))
```